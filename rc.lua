...
globalkeys = gears.table.join(
    ...
    awful.key({ modkey }, "p",
              function ()
                  awful.prompt.run {
                    prompt = "Copy a password: ",
                    textbox = awful.screen.focused().mypromptbox.widget,
                    history_path = awful.util.getdir("cache") .. "/history_pass",
                    exe_callback = function (s)
                        local function on_output (line)
                            naughty.notify({ title = line:gsub("^(.-)%s*$", "%1"),
                            preset = naughty.config.presets.normal })
                         end

                         awful.spawn.with_line_callback("bash -c 'pass -c " .. s .. " 2>&1'", {
                            stdout = on_output,
                            stderr = on_output
                         })
                    end
              ...
              {description = "copy pass prompt", group = "awesome"}),
    -- Open a npm package page in a browser
    awful.key({ modkey, "Shift" }, "n",
              function ()
                  awful.prompt.run {
                    prompt = "Open a npm package page: ",
                    textbox = awful.screen.focused().mypromptbox.widget,
                    history_path = awful.util.getdir("cache") .. "/history_npm_package_page",
                    exe_callback = function (s)
                        awful.util.spawn("xdg-open https://www.npmjs.com/package/" .. s)
                    end,
                  }
               end),
    awful.key({ modkey }, "\\",
...
clientkeys = gears.table.join(
    ...
    awful.key({ modkey, "Control"   }, "Escape", function (c) c:kill()                       end,
             {description = "close", group = "client"}),
